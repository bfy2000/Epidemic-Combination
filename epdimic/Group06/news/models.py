...
class Column(models.Model):
    ...
    parent_cl_id = models.ForeignKey('news.Column', on_delete=models.SET_NULL, null=True, default=None)  # 父栏目id，可以为空
...
class PublishComments(models.Model):    # 用户发表评论表
    ...
    user_id = models.ForeignKey('users.UserInfo', on_delete=models.SET_NULL, null=True)   # 用户为空可以显示用户已注销或不存在，不需要删除记录
    cmt_id = models.ForeignKey('news.Comment', on_delete=models.CASCADE)
...
class NewsFiles(models.Model):  # 新闻文件关系
    ...
    news_id = models.ForeignKey('news.News', on_delete=models.CASCADE)
    files_id = models.ForeignKey('news.Files', on_delete=models.CASCADE)


class NewsImages(models.Model):     # 新闻图片关系
    news_files_id = models.BigAutoField(primary_key=True)
    news_id = models.ForeignKey('news.News', on_delete=models.CASCADE)
    img_id = models.ForeignKey('news.Images', on_delete=models.CASCADE)


class NewsColumn(models.Model):     # 新闻栏目关系
    news_cl_id = models.BigAutoField(primary_key=True)
    news_id = models.ForeignKey('news.News', on_delete=models.CASCADE)
    cl_id = models.ForeignKey('news.Column', on_delete=models.CASCADE)